...
const useScrollReveal = () => {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      // Reveal before the section enters the viewport so text is never missing
  ...
  return (
    <div ref={ref} className={`group flex items-start space-x-4 transition-all duration-700 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-[-50px]'}`}>
      ...
      <div className="flex-1">
        ...
        <h4 className="font-display text-xl font-bold text-primary-dark group-hover:text-accent-blue transition-colors mb-2">
          {title}
        </h4>
...
const StatsCounter = ({ target, suffix = '', label }: { target: number; suffix?: string; label: string }) => {
  ...
  return (
    <div ref={ref} className="text-center group hover-scale">
      <div className="font-display text-4xl md:text-5xl font-black mb-2 text-accent-blue">
...
const About = () => {
  ...
  return (
    <div className="min-h-screen bg-primary-light">
      {/* Enhanced Header */}
      <header
        className={`bg-primary-dark shadow-lg sticky top-0 z-50 transition-all duration-300 ${headerShrunk ? 'py-1 shadow-xl' : 'py-0'}`}
        ...
        <div className="container mx-auto px-4">
          <div className={`flex justify-between items-center transition-all duration-300 ${headerShrunk ? 'py-2' : 'py-4'}`}>
            <div 
              ...
              onClick={scrollToTop}
            >
              <div className={`bg-accent-blue rounded-lg flex items-center justify-center transition-all duration-300 subtle-glow flex-shrink-0 ${headerShrunk ? 'w-9 h-9 sm:w-10 sm:h-10' : 'w-10 h-10 sm:w-12 sm:h-12'}`}>
                <span className={`text-primary-light font-bold transition-all ${headerShrunk ? 'text-base sm:text-lg' : 'text-lg sm:text-xl'}`}>PLT</span>
              </div>
              <div>
                <h1 className={`font-bold text-primary-light group-hover:text-accent-blue transition-colors font-display ${headerShrunk ? 'text-base sm:text-xl' : 'text-lg sm:text-2xl'}`}>
            ...
            <nav className="py-4 border-t border-primary-light/20 flex flex-col space-y-2">
              {[
                ...
                <NavLink
                  ...
                  onClick={(e) => { e.preventDefault(); setIsMenuOpen(false); navigate(item.to); }}
                >
                  {item.label}
                </NavLink>
              ))}
      ...
      >
        <div className={`container mx-auto text-center relative z-10 transition-all duration-500 ${heroReveal.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          {/* Animated Badge */}
          <div className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-gradient-to-r from-accent-blue/10 to-secondary/10 border border-accent-blue/20 backdrop-blur-sm mb-8 hover-scale">
            <Sparkles className="w-5 h-5 text-accent-blue" />
            <span className="font-semibold text-lg text-accent-blue">Trusted Since 2008</span>
          </div>

          <h1 className="font-display text-5xl md:text-7xl font-black mb-6 leading-tight drop-shadow-lg">
            About{' '}
            <span className="text-accent-blue inline-block">PowerLink Technologies</span>
          </h1>
          ...
          <p className="text-xl md:text-2xl text-primary-dark/80 mb-12 max-w-4xl mx-auto leading-relaxed">
            ...
            <span className="text-accent-blue font-semibold">power solutions</span> and security systems for over 18 years, serving 1500+ satisfied customers across Bangalore.
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-16">
            <Button
              ...
              size="lg"
              className="executive-gradient text-primary-light font-semibold px-10 py-6 rounded-2xl professional-shadow group relative overflow-hidden shadow-2xl"
      ...
      >
        <div className={`container mx-auto transition-all duration-500 ${storyReveal.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-16">
              <h3 className="font-display text-4xl md:text-5xl font-black mb-4 text-primary-dark">
                Our Journey{' '}
                <span className="text-accent-blue">Timeline</span>
              </h3>
      ...
      >
        ...
        <div className={`container mx-auto relative z-10 transition-all duration-500 ${statsReveal.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-6xl mx-auto">
            ...
            <StatsCounter target={24} suffix="/7" label="Support Available" />
          </div>
        </div>
      </section>
      ...
      >
        <div className={`container mx-auto transition-all duration-500 ${missionReveal.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-stretch max-w-6xl mx-auto">
            <Card className="bg-white border border-border shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
              <CardContent className="p-10">
                ...
                <h3 className="font-display text-3xl font-black mb-6 text-accent-blue">Our Mission</h3>
            ...
            <Card className="bg-white border border-border shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
              <CardContent className="p-10">
                ...
                <h3 className="font-display text-3xl font-black mb-6 text-accent-blue">Our Vision</h3>
                <p className="text-xl text-primary-dark/80 leading-relaxed">
      ...
      <section 
        ...
        className="py-24 px-4 bg-primary-dark text-primary-light relative overflow-hidden"
      >
        <div className={`container mx-auto text-center transition-all duration-500 ${valuesReveal.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <h3 className="font-display text-4xl md:text-5xl font-black mb-16 text-primary-light">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 max-w-6xl mx-auto">
            {[
              ...
              >
                <div className={`w-24 h-24 bg-gradient-to-br ${value.color} rounded-2xl flex items-center justify-center mx-auto mb-8 shadow-2xl group-hover:scale-110 transition-all duration-500`}>
                  <value.icon className="w-12 h-12 text-white" />
                </div>
                <h4 className="font-display text-2xl font-black mb-4 text-primary-light group-hover:text-accent-blue transition-colors">
                  {value.title}
                </h4>
      ...
      >
        ...
        <div className="container mx-auto relative z-10 text-center">
          <div className={`transition-all duration-500 ${ctaReveal.isVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-12 scale-95'}`}>
            <h3 className="font-display text-4xl md:text-5xl font-black mb-6 text-primary-light drop-shadow-lg">
              Ready to Power Your{' '}
              <span className="text-accent-blue">Future?</span>
            </h3>
            ...
            <div className="flex flex-col sm:flex-row gap-6 justify-center">
              <Button
                ...
                size="lg"
                className="executive-gradient text-primary-light font-black text-lg px-12 py-8 rounded-2xl professional-shadow shadow-2xl group transition-all duration-300"
                onClick={handleWhatsAppContact}
              >
